...
This executable combines `go-postgres`, pgx's `database/sql` bridge, and
`go-migrations`. It plans and applies embedded migrations once as a dedicated
deployment job:
...
The example is a nested module because the current `go-migrations` release
requires Go 1.26.5 while `go-postgres` supports Go 1.25. The local `replace`
