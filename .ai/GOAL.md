...
- pgx tracing adapters compatible with `go-telemetry`
- pool acquisition, saturation, query, transaction, and error metrics
- optional `go-log`/`slog` integration with safe SQL/argument policy
...
- deterministic database setup hooks compatible with `go-migrations`
...
- no replacement for pgx, `sqlc`, or `go-migrations`
...
- `go-migrations` integration and Kubernetes migration-job example
...
4. Build `sqlc`, `go-migrations`, service, and worker examples.
