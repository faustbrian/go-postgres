...
- `RunTransaction` invokes the closure once, never retries it, commits success,
  rolls back error or panic with an uncanceled bounded cleanup context, joins
  callback and rollback failures, and re-panics the original panic value.
...
- `postgrestest.Start` owns a real PostgreSQL container, waits for readiness,
  ...
  `HostPort` supports stable-endpoint stop/restart tests.
