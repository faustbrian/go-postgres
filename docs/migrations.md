# go-migrations integration

`go-postgres` does not run migrations during service startup. Schema changes
...
Use the executable [`go-migrations` example](../examples/migrations/) with its
`database/sql` PostgreSQL adapter and the same secret source as the service.
It constructs the owned `go-postgres` pool, exposes it through pgx's standard
