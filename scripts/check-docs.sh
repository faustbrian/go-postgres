...
bash scripts/extract-release-notes.sh 1.0.0 > "${TMPDIR:-/tmp}/go-postgres-release-notes.md"
test -s "${TMPDIR:-/tmp}/go-postgres-release-notes.md"
! grep -q '^\[[^]]*\]:' "${TMPDIR:-/tmp}/go-postgres-release-notes.md"
go test ./... -run 'TestExportedSymbolsHaveGoDocumentation|^Example'
go build ./examples/...
(cd examples/migrations && go mod tidy -diff && go test .)
